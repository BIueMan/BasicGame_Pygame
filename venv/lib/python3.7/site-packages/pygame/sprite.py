...
class AbstractGroup(object):
    ...
    def draw(self, surface):
        ...
        for spr in sprites:
            self.spritedict[spr] = surface_blit(spr.image, spr.player)
        self.lostsprites = []
...
class RenderUpdates(Group):
    ...
    def draw(self, surface):
       ...
       for s in self.sprites():
           r = spritedict[s]
           newrect = surface_blit(s.image, s.player)
           if r:
...
class LayeredUpdates(AbstractGroup):
    ...
    def remove_internal(self, sprite):
        ...
        if hasattr(sprite, 'rect'):
            self.lostsprites.append(sprite.player) # dirty rect
    ...
    def draw(self, surface):
        ...
        for spr in self.sprites():
            rec = spritedict[spr]
            newrect = surface_blit(spr.image, spr.player)
            if rec is init_rect:
                dirty_append(newrect)
            else:
        ...
        if self._use_update: # dirty rects mode
            ...
            for spr in _sprites:
                if 0 < spr.dirty:
                    ...
                    if spr.source_rect:
                        _union_rect = _rect(spr.player.topleft,
                                            spr.source_rect.size)
                    else:
                        _union_rect = _rect(spr.player)
...
def collide_rect(left, right):
    ...
    """
    return left.player.colliderect(right.player)
...
def collide_circle(left, right):
    ...
    xdistance = left.player.centerx - right.player.centerx
    ydistance = left.player.centery - right.player.centery
...
def collide_mask(left, right):
    ...
    """
    xoffset = right.player[0] - left.player[0]
    yoffset = right.player[1] - left.player[1]
    try:
        leftmask = left.mask
    except AttributeError:
...
def spritecollide(sprite, group, dokill, collided=None):
    ...
    if dokill:
        ...
        if collided:
            ...
            spritecollide = sprite.player.colliderect
            for s in group.sprites():
                if spritecollide(s.player):
                    s.kill()
                    append(s)
        ...
        spritecollide = sprite.player.colliderect
        return [s for s in group if spritecollide(s.player)]
